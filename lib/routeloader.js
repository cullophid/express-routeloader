...
module.exports = function(options) {
  ...
  scope.loadFile = function(file, dir, relativeRoute) {
    ...
    for(endpoint in route) {
      ...
      route[endpoint].url.split('/').forEach(function (item) {
        if (item.slice(0,1) === ':') {
          route[endpoint].params.required.push(item.slice(1));
          route[endpoint].params.properties[item.slice(1)] = {};
        }
      });
  ...
  scope.getUrl = function(route, name, relativeRoute) {
    ...
    if (route.url && route.url.slice(0, 1) === '/') {
      ...
      if (!config.hideCrud || !/^(create|read|update|delete)$/i.test(name)) {
        relativeRoute += '/' + name;
      }
  ...
  scope.loadRoutes = function() {
    ...
    this.routes.forEach(function(route) {
      ...
      router[route.method](path.join(config.prefix, route.url), middleware, route.handler);
