...
describe('When loading a directory', function() {
    describe('with a single file', function() {
        var router, route;
        before(function () {
            router = routeloader({rootDir : './test/routes/loadDir/single'});
    ...
    describe('with all crud types', function() {
        var router, routes;
        before(function () {
            router = routeloader({rootDir : './test/routes/loadDir/crud'});
    ...
    describe('with a custom url and method', function() {
        var router, routes;
        before(function () {
            router = routeloader({rootDir : './test/routes/loadDir/custom'});
            routes = router._scope.routes;
        });
    ...
    describe('with an invalid route', function() {
        ...
        before(function () {
            try {
                routeloader({rootDir : './test/invalid'});
            } catch (e) {
                error = e;
            }
        });
        ...
        it('should throw a friendly error', function () {
            error.message.should.equal('/asset/create.js does not export a function');
        });
